...
class MenuHelper extends AppHelper{
    ...
    public $settings = array(
        'parentFormat' => '<ul%s>%s</ul>',
        'childFormat' => '<li%s>%s%s</li>',
        'activeClass' => 'active'
    );
    ...
    protected function buildItems($items){
        ...
        foreach($items as $item){
            ...
            $url = isset($item['url']) ? $this->url($item['url']['url']) : null;
            ...
            if(isset($item['items']) && is_array($item['items'])){
                ...
                if($item['access'] === true){
                    if(isset($item['url'])){
                        $urlOptions = isset($item['url']['options']) ? $item['url']['options'] : null;

                        $renderedItems = '';
                        foreach ($item['items'] as $nestedItem) {
                            $renderedItems .= $this->render($nestedItem);
                        }

                        $li = $this->Html->tag('li', $this->Html->link($item['url']['title'], 
                            $item['url']['url'], $urlOptions).$renderedItems, $options);
                    }
                    else{
            ...
            elseif($item['access'] === true){
                if(isset($item['url'])){
                    ...
                    $li = $this->Html->tag('li', $this->Html->link($item['url']['title'], 
                            $item['url']['url'], $urlOptions), $options);
                }
                else{
